...
append_normalized_escapes(struct strbuf *buf, const char *from, size_t from_len,
			  const char *esc_extra, const char *esc_ok)
{
	/*
...
static int
match_host(const struct url_info *url_info, const struct url_info *pattern_info)
{
...
static char *
url_normalize_1(const char *url, struct url_info *out_info, char allow_globs)
{
	/*
...
static size_t
url_match_prefix(const char *url, const char *url_prefix, size_t url_prefix_len)
{
	/*
...
static int
cmp_matches(const struct urlmatch_item *a, const struct urlmatch_item *b)
{
