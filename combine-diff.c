...
intersect_paths(struct combine_diff_path *curr, int n, int num_parent)
{
...
static struct lline *
coalesce_lines(struct lline *base, int *lenbase, struct lline *new, int lennew,
...
static char *
grab_blob(const struct object_id *oid, unsigned int mode, unsigned long *size,
...
static void consume_line(void *state_, char *line, unsigned long len)
{
	...
	if (5 < len && !memcmp("@@ -", line, 4)) {
		if (parse_hunk_header(line, len, &state->ob, &state->on,
				      &state->nb, &state->nn))
			return;
		state->lno = state->nb;
		if (state->nn == 0) {
...
static void
combine_diff(const struct object_id *parent, unsigned int mode,
	     mmfile_t *result_file, struct sline *sline, unsigned int cnt,
	     int n, int num_parent, int result_deleted,
	     struct userdiff_driver *textconv, const char *path, long flags)
{
	...
	if (xdi_diff_outf(&parent_file, result_file, consume_line, &state, &xpp,
			  &xecfg))
		die("unable to generate combined diff for %s", oid_to_hex(parent));
...
static unsigned long adjust_hunk_tail(struct sline *sline, unsigned long all_mask,
...
find_next(struct sline *sline, unsigned long mark, unsigned long i,
	  unsigned long cnt, int look_for_uninteresting)
{
...
static int
make_hunks(struct sline *sline, unsigned long cnt, int num_parent, int dense)
{
	unsigned long all_mask = (1UL << num_parent) - 1;
	unsigned long mark = (1UL << num_parent);
	unsigned long i;
...
static void
dump_sline(struct sline *sline, const char *line_prefix, unsigned long cnt,
	   int num_parent, int use_color, int result_deleted)
{
	unsigned long mark = (1UL << num_parent);
	unsigned long no_pre_delete = (2UL << num_parent);
	int i;
...
static void
reuse_combine_diff(struct sline *sline, unsigned long cnt, int i, int j)
{
...
static void
show_combined_header(struct combine_diff_path *elem, int num_parent, int dense,
		     ...
		     int mode_differs, int show_file_header)
{
	...
	for (i = 0; i < num_parent; i++) {
		abb = find_unique_abbrev(elem->parent[i].oid.hash, abbrev);
		printf("%s%s", i ? "," : "", abb);
	}
	...
	if (added)
		...
		dump_quoted_path("--- ", a_prefix, elem->path, line_prefix,
				 c_meta, c_reset);
	if (deleted)
		dump_quoted_path("+++ ", "", "/dev/null", line_prefix, c_meta,
				 c_reset);
...
static void show_patch_diff(struct combine_diff_path *elem, int num_parent,
			    int dense, int working_tree_file, struct rev_info *rev)
{
	struct diff_options *opt = &rev->diffopt;
	...
	if (!working_tree_file)
		...
		if (S_ISLNK(st.st_mode)) {
			...
			if (is_file) {
				...
				if (convert_to_git(&the_index, elem->path, result,
						   len, &buf, safe_crlf)) {
					free(result);
	...
	if (textconv)
		...
		for (i = 0; !is_binary && i < num_parent; i++) {
			...
			buf = grab_blob(&elem->parent[i].oid,
					elem->parent[i].mode, &size, NULL, NULL);
	...
	for (i = 0; i < num_parent; i++) {
		int j;
		for (j = 0; j < i; j++) {
			if (!oidcmp(&elem->parent[i].oid, &elem->parent[j].oid)) {
				reuse_combine_diff(sline, cnt, i, j);
				break;
			}
		}
		if (i <= j)
			combine_diff(&elem->parent[i].oid, elem->parent[i].mode,
				     &result_file, sline, cnt, i, num_parent,
				     result_deleted, textconv, elem->path,
				     opt->xdl_opts);
	}
...
static void
show_raw_diff(struct combine_diff_path *p, int num_parent, struct rev_info *rev)
{
	struct diff_options *opt = &rev->diffopt;
 ...
 */
static struct diff_filepair *
combined_pair(struct combine_diff_path *p, int num_parent)
{
	int i;
