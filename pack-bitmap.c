...
static struct ewah_bitmap *read_bitmap_1(struct bitmap_index *index)
{
	...
	int bitmap_size = ewah_read_mmap(b, index->map + index->map_pos,
...
static int load_bitmap_header(struct bitmap_index *index)
{
	...
	/* Parse known bitmap format options */
	{
		...
		if (flags & BITMAP_OPT_HASH_CACHE) {
			...
			index->hashes =
				((uint32_t *)end) - index->pack->num_objects;
		}
	}
...
store_bitmap(struct bitmap_index *index, struct ewah_bitmap *root,
	     const unsigned char *sha1, struct stored_bitmap *xor_with,
	     int flags)
{
...
static int load_bitmap_entries_v1(struct bitmap_index *index)
{
	...
	for (i = 0; i < index->entry_count; ++i) {
		...
		recent_bitmaps[i % MAX_XOR_OFFSET] =
			store_bitmap(index, bitmap, sha1, xor_bitmap, flags);
	}
...
static int open_pack_bitmap_1(struct packed_git *packfile)
{
	...
	bitmap_git.map =
		xmmap(NULL, bitmap_git.map_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
static int ext_index_add_object(struct object *object, const char *name)
{
	...
	hash_pos =
		kh_put_sha1_pos(eindex->positions, object->oid.hash, &hash_ret);
	if (hash_ret > 0) {
...
static int add_to_include_set(struct include_data *data,
			      const unsigned char *sha1, int bitmap_pos)
{
	khiter_t hash_pos;
...
static int should_include(struct commit *commit, void *_data)
{
	...
	if (bitmap_pos < 0)
		bitmap_pos =
			ext_index_add_object((struct object *)commit, NULL);
...
find_objects(struct rev_info *revs, struct object_list *roots,
	     struct bitmap *seen)
{
	...
	while (roots) {
		...
		if (object->type == OBJ_COMMIT) {
			khiter_t pos = kh_get_sha1(bitmap_git.bitmaps,
						   object->oid.hash);
...
static void
show_extended_objects(struct bitmap *objects, show_reachable_fn show_reach)
{
	struct eindex *eindex = &bitmap_git.ext_index;
	uint32_t i;
	...
	for (i = 0; i < eindex->count; ++i) {
		...
		show_reach(obj->oid.hash, obj->type, 0, eindex->hashes[i], NULL,
			   0);
	}
}

static void
show_objects_for_type(struct bitmap *objects, struct ewah_bitmap *type_filter,
				       ...
				       uint32_t *entries, off_t *up_to)
{
	...
	if (reuse_objects >= bitmap_git.pack->num_objects) {
		bitmap_git.reuse_objects = *entries =
			bitmap_git.pack->num_objects;
...
void traverse_bitmap_commit_list(show_reachable_fn show_reachable)
{
	...
	show_objects_for_type(bitmap_git.result, bitmap_git.commits, OBJ_COMMIT,
			      show_reachable);
	show_objects_for_type(bitmap_git.result, bitmap_git.trees, OBJ_TREE,
			      show_reachable);
	show_objects_for_type(bitmap_git.result, bitmap_git.blobs, OBJ_BLOB,
			      show_reachable);
	show_objects_for_type(bitmap_git.result, bitmap_git.tags, OBJ_TAG,
			      show_reachable);
...
static void
test_show_object(struct object *object, const char *name, void *data)
{
...
static int rebuild_bitmap(uint32_t *reposition, struct ewah_bitmap *source,
			  struct bitmap *dest)
{
	uint32_t pos = 0;
	struct ewah_iterator it;
