 ...
 */
static void
range_set_union(struct range_set *out, struct range_set *a, struct range_set *b)
{
 ...
 */
static void
diff_ranges_filter_touched(struct diff_ranges *out, struct diff_ranges *diff,
			   struct range_set *rs)
{
	unsigned int i, j = 0;
...
static struct line_log_data *
line_log_data_merge(struct line_log_data *a, struct line_log_data *b)
{
...
static struct line_log_data *
lookup_line_range(struct rev_info *revs, struct commit *commit)
{
	struct line_log_data *ret = NULL;
	struct line_log_data *d;
		   ...
		   struct string_list *args)
{
	...
	if (!rev->diffopt.detect_rename) {
		...
		parse_pathspec(&rev->diffopt.pathspec, 0, PATHSPEC_PREFER_FULL,
			       "", paths);
...
static void
move_diff_queue(struct diff_queue_struct *dst, struct diff_queue_struct *src)
{
	assert(src != dst);
...
static void
print_line(const char *prefix, char first, long line, unsigned long *ends,
	   void *data, const char *color, const char *reset, FILE *file)
{
...
static int
process_all_files(struct line_log_data **range_out, struct rev_info *rev,
		  struct diff_queue_struct *queue, struct line_log_data *range)
{
	int i, changed = 0;
...
static int
process_ranges_ordinary_commit(struct rev_info *rev, struct commit *commit,
			       struct line_log_data *range)
{
...
static int
process_ranges_arbitrary_commit(struct rev_info *rev, struct commit *commit)
{
	struct line_log_data *range = lookup_line_range(rev, commit);
	int changed = 0;
...
line_log_rewrite_one(struct rev_info *rev, struct commit **pp)
{
	for (;;) {
		struct commit *p = *pp;
